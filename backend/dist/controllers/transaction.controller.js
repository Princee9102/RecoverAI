...
exports.generateTransactions = exports.getTransactionById = exports.getTransactions = void 0;
...
const getTransactionById = async (req, res) => {
    try {
        const { id } = req.params;
        const txn = await prisma_1.prisma.transaction.findFirst({
            where: {
                OR: [
                    { id },
                    { transaction_id: id }
...
const generateTransactions = async (req, res) => {
    try {
        const seedScript = path_1.default.join(__dirname, '../../prisma/seed.ts');
        (0, child_process_1.exec)(`npx ts-node "${seedScript}"`, (error, stdout, stderr) => {
            if (error) {
        ...
        res.status(500).json({ error: 'Failed to trigger seed script', details: err.message });
    }
};
