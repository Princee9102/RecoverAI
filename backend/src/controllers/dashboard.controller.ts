...
export const getDashboardMetrics = async (req: Request, res: Response) => {
  try {
    ...
    const revenueAtRiskAgg = await prisma.transaction.aggregate({
      _sum: { amount: true }
    });
    const revenueAtRisk = revenueAtRiskAgg._sum.amount || 0;

    const recoverableAgg = await prisma.transaction.aggregate({
      where: { recovery_status: { in: ['recoverable', 'recovering', 'recovered'] } },
      _sum: { amount: true }
    });
    const recoverableRevenue = recoverableAgg._sum.amount || 0;

    const recoveredAgg = await prisma.transaction.aggregate({
    ...
    const recoverableCount = await prisma.transaction.count({
      where: { recovery_status: { in: ['recoverable', 'recovering', 'recovered'] } }
    });

    const humanEscalations = await prisma.transaction.count({
    ...
    const failureReasonBreakdown = await Promise.all(
      failureReasons.map(async (reason) => {
        const total = await prisma.transaction.count({ where: { failure_reason: reason } });
        const recovered = await prisma.transaction.count({ where: { failure_reason: reason, recovery_status: 'recovered' } });
    ...
    const statusDistribution = await Promise.all(
      statuses.map(async (status) => {
        const count = await prisma.transaction.count({ where: { recovery_status: status } });
        return {
    ...
    const actionDistribution = await Promise.all(
      actions.map(async (action) => {
        const count = await prisma.transaction.count({ where: { recovery_action: action } });
        return {
    ...
    res.status(500).json({ error: 'Failed to calculate dashboard metrics', details: (err as Error).message });
  }
};
