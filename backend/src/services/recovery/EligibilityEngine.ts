...
export interface EligibilityResult {
  ...
  score: number;
  reason: string;
  category: 'high_priority' | 'standard' | 'low_priority' | 'ineligible';
}

export class EligibilityEngine {
  public static evaluateEligibility(txn: Transaction): EligibilityResult {
    ...
    let category: 'high_priority' | 'standard' | 'low_priority' | 'ineligible' = 'standard';
    if (score >= 0.75) category = 'high_priority';
    else if (score >= 0.45) category = 'standard';
