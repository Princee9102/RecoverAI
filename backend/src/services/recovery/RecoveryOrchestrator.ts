...
import { SafetyPolicyGate } from './SafetyPolicyGate';
import { EligibilityEngine } from './EligibilityEngine';
import { Transaction } from '@prisma/client';
import { AllowedAction } from '../../schemas/recovery.schema';
...
export interface BatchSimulationResult {
  ...
  recoveryRate: number; // percentage of recoverable
...
export class RecoveryOrchestrator {
  public static async analyzeTransaction(transactionId: string) {
    ...
    const eligibility = EligibilityEngine.evaluateEligibility(txn);
  ...
  public static async executeRecoveryAction(transactionId: string, manualActionOverride?: AllowedAction) {
    ...
    const actionToEvaluate = manualActionOverride || (txn.recovery_action as AllowedAction) || 'schedule_retry';

    // 1. Safety Policy Gate Check
    const safetyCheck = SafetyPolicyGate.evaluate(txn, actionToEvaluate);
  ...
  public static async simulateBatch(): Promise<BatchSimulationResult> {
  ...
  public static async runFailureDemo() {
    // Pick or create a target transaction specifically for Gateway Timeout scenario
    let demoTxn = await prisma.transaction.findFirst({
      where: { failure_reason: 'bank_timeout' }
    });

    if (!demoTxn) {
      demoTxn = await prisma.transaction.create({
        data: {
      ...
      demoTxn = await prisma.transaction.update({
        where: { id: demoTxn.id },
        data: {
          recovery_attempts: 1,
          communication_attempts: 2,
