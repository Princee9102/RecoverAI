...
import { AllowedAction, AllowedActionsSchema } from '../../schemas/recovery.schema';

export interface SafetyCheckResult {
  isAllowed: boolean;
  actionToExecute: AllowedAction;
  policyReason: string;
  ruleTriggered?: string;
}

export class SafetyPolicyGate {
  private static MAX_PAYMENT_RETRIES = 2;
  private static MAX_RECOVERY_COMMS = 3;

  public static evaluate(txn: Transaction, proposedAction: AllowedAction): SafetyCheckResult {
    // Rule 1: Validate proposed action against strict ALLOWED_ACTIONS enum list
    const parseResult = AllowedActionsSchema.safeParse(proposedAction);
